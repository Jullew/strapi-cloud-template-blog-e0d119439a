...
const robotData = require('../robots/tesla.json');
const claimsData = require('../robots/tesla-claims.json');
...
async function determineEndpoint(collection) {
    ...
    // Special cases
    if (collection === 'environment-types') {
        endpointCache[collection] = 'environment-types';
        return 'environment-types';
    }
    if (collection === 'target-users') {
        endpointCache[collection] = 'target-users';
        return 'target-users';
    }
...
async function main() {
    try {
        ...
        if (claimsData.sources_to_create) {
            for (const sData of claimsData.sources_to_create) {
                let source = await findFirst('sources', { title: sData.title });
                if (!source) {
                    console.log(`Creating Source: ${sData.title}`);
                    source = await fetchAPI(`/${sourceCollection}`, 'POST', {
                        title: sData.title,
                        // slug: sData.slug, // Not in schema yet
                ...
                sourceMap[sData.slug] = source.id;
            }
        }
        ...
        if (!robot) {
            ...
            robot = await fetchAPI(`/${robotCollection}/${robot.documentId}`, 'PUT', baseRobotData);
        }
